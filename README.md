...
バックグラウンドで、ターミナルを閉じても動かし続けたい場合は:

```bash
nohup ./serve.py > /tmp/nagi.log 2>&1 & disown
```

止めるときは `ps aux | grep serve.py` で PID を探して `kill <PID>` してください。
...
原文側の言語は「自動検出」も選べます（[franc](https://github.com/wooorm/franc) による判定。短すぎる文や franc が対応していない言語は検出できません）。
